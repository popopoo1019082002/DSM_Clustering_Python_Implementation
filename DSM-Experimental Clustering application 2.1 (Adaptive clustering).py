...
def get_user_input():
    sub_systems = []
    while True:
        num_sub_systems = input("Enter the number of sub-systems (or type 'exit' to stop): ")
        if num_sub_systems.lower() == 'exit':
            break
        try:
            num_sub_systems = int(num_sub_systems)
            if num_sub_systems < 1:
                print("The number of sub-systems must be at least 1. Please try again.")
                continue
            for i in range(num_sub_systems):
                sub_system = input(f"Enter sub-system {i+1}: ")
                if not sub_system.strip():
                    raise ValueError("Sub-system name cannot be empty.")
