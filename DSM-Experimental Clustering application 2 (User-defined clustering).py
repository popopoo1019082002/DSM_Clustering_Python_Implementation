...
def perform_clustering(dsm_matrix, sub_systems, cluster_numbers):
    """
    Performs clustering on the DSM matrix using the number of clusters provided by the user.

    Args:
        dsm_matrix (numpy.ndarray): The DSM matrix.
        sub_systems (list): List of sub-system names.
        cluster_numbers (dict): Dictionary with the number of clusters for each clustering method.
    ...
    kmeans = KMeans(n_clusters=cluster_numbers['kmeans'], random_state=42)
    clustering_results['kmeans'] = kmeans.fit_predict(X)

    # Perform Spectral clustering with user-defined number of clusters
    spectral = SpectralClustering(n_clusters=cluster_numbers['spectral'], random_state=42, affinity='nearest_neighbors')
    clustering_results['spectral'] = spectral.fit_predict(X)

    # Perform Agglomerative clustering with user-defined number of clusters
    agglomerative = AgglomerativeClustering(n_clusters=cluster_numbers['agglomerative'])
    clustering_results['agglomerative'] = agglomerative.fit_predict(X)

    return clustering_results


def get_user_input_with_clusters():
    """
    Collects user input for sub-systems, interfaces, and the number of clusters for each clustering method.
    ...
    # Collect the number of clusters for each method
    cluster_numbers['kmeans'] = int(input("Enter the number of clusters for K-means: "))
    cluster_numbers['spectral'] = int(input("Enter the number of clusters for Spectral Clustering: "))
    cluster_numbers['agglomerative'] = int(input("Enter the number of clusters for Agglomerative Clustering: "))

    return sub_systems, interfaces, cluster_numbers
...
# Collect user input for sub-systems, interfaces, and number of clusters for each method
sub_systems, interfaces, cluster_numbers = get_user_input_with_clusters()

# Create the initial DSM matrix based on the user input
dsm_matrix = create_dsm_matrix(sub_systems, interfaces)

# Perform clustering on the DSM matrix with the user-defined number of clusters
cluster_labels = perform_clustering(dsm_matrix, sub_systems, cluster_numbers)
